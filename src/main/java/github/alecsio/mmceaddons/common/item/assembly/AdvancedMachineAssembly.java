package github.alecsio.mmceaddons.common.item.assembly;
...
import github.alecsio.mmceaddons.common.item.assembly.handler.exception.MultiblockBuilderNotFoundException;
 ...
 * Assemblies are managed by {@link MachineAssemblyManager}, which acts as
...
public class AdvancedMachineAssembly extends AbstractMachineAssembly {
    ...
    private void tryPlaceFluid(StructureIngredient.FluidIngredient fluidIngredient) throws MultiblockBuilderNotFoundException {
        ...
        for (int i = fluidIngredient.ingredientList().size() - 1; i >= 0; i--) {
            ...
            if (handledFluid == null && Mods.APPLIEDENERGISTICS.isPresent() && mode.supports(AssemblySupportedMods.APPLIEDENERGISTICS2)) {
    ...
    private void tryPlaceBlock(StructureIngredient.ItemIngredient ingredientToProcess) throws MultiblockBuilderNotFoundException {
        ...
        for (int i = ingredientToProcess.ingredientList().size() - 1; i >= 0; i--) {
            ...
            if (handledItem == EMPTY && Mods.APPLIEDENERGISTICS.isPresent() && mode.supports(AssemblySupportedMods.APPLIEDENERGISTICS2)) {
                handledItem = canMEHandle(stack, assembler);
            }

            if (handledItem == EMPTY && Mods.PROJECTE.isPresent() && mode.supports(AssemblySupportedMods.PROJECTE)) {
